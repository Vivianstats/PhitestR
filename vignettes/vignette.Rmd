---
title: "Introduction to Phitest"
author: "Wei Vivian Li, Rutgers Department of Biostatistics and Epidemiology"
date: "`r Sys.Date()`"
#output: rmarkdown::github_document
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Phitest: Analyzing the Heterogeneity of Single-Cell Populations}
...
`Phitest` currently supports two types of input: a UMI count matrix with cluster labels, or a Seurat object with cluster information.

### Run `Phitest` with a count matrix

To demonstrate the usage of `Phitest` with a count matrix, we use an example Cortex dataset which can be downloaded from this [link](https://rutgers.box.com/s/57cpndrph9h1cxyment4k2meg76y0lqx). To use `Phitest`, we need a count matrix (in this example, `object`) with rows representing genes and columns representing cells. In addition, we also need a (character or numeric) vector of labels (in this case, `label`) specifying the assignments of the cells.
```{r eval = FALSE}
